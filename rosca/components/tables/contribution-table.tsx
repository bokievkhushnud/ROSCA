...
export default function ContributionTable({ contributions, users, loans }: ContributionTableProps) {
  ...
  const filteredContributions = contributions.filter((contribution) =>
    ...
    ) || (contribution.user && Object.values(contribution.user).some(
      (userValue) => userValue && String(userValue).toLowerCase().includes(searchTerm.toLowerCase())
    ))
  );
  ...
  return (
    <div className="space-y-6">
      <section className="bg-white rounded-lg shadow-sm p-6">
        <div className="mb-6">
          <h2 className="text-xl font-semibold text-gray-900">Contribution Management</h2>
          <p className="text-sm text-gray-500 mt-1">Track and manage member contributions</p>
        </div>
        ...
        <div className="space-y-4">
                    ...
                    <tr key={contribution.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full 
                          ${contribution.status === 'RECEIVED' ? 'bg-green-100 text-green-800' : 
          ...
          <div className="flex justify-between items-center">
            <div className="text-sm text-gray-700">
              Showing {startIndex + 1} to {Math.min(startIndex + itemsPerPage, filteredContributions.length)} of{" "}
              {filteredContributions.length} results
            </div>
